...
- V0.9 成就首次达成会获得因果奖励，结算页展示积分来源明细，所有奖励与积分由服务端计算
- 生命、灵力、寿元、因果属性变化
- 存档到 MySQL
...
V0.9 成就达成奖励写入 `achievement_record.reward_causality`，首次达成时计入账号因果点；历史 V0.8 记录只补充默认字段，不重复追发奖励。结算 `scoreBreakdown` 返回进度、回合、属性、资源、构筑、精英和飞升加分，明细总和与 `score` 一致。迁移文件为 `database/migrations/20260726_v09_achievement_rewards.sql`。
